...
class DB:
    ...
    def create(query):
        ...
        if not SQL_query_params["username"] or not SQL_query_params["uniq_key"]:
            return {"response": "create method must include username and uniq_key"}
        SQL_query = f"""
    ...
    @staticmethod
    def update(query):
        #http://127.0.0.1:8000/api/request?table=users&method=update&id=5&username=test:tochange&uniq_key=&role=
        # Создаем словарь полей для запроса к БД
